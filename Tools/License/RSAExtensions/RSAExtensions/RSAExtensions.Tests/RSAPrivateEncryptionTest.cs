...
namespace RSAExtensions.Tests
{
    ...
    public class RSAPrivateEncryptionTest
    {
        ...
        public void PrivareEncryption_Throws_Exception_On_Large_Data_Length()
        {
            RSACryptoServiceProvider rsa = new RSACryptoServiceProvider(1024);
            Random rnd = new Random();
            byte[] data = new byte[123];
            rnd.NextBytes(data);
            byte[] encData = rsa.PrivareEncryption(data);
        }

        [TestMethod]
        [ExpectedException(typeof(InvalidOperationException))]
        public void PrivareEncryption_Throws_Exception_On_PublicKey_Only()
        {
            // Public key:
